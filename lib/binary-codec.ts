class Context {
        ...
        switch (typeof val) {
            ...
            case "function":
                return `${this.addImport(
                    val,
                )}.call(${this.generateVariable()}, vars)`;
        }
    }
    ...
    getUnresolvedReferences(): string[] {
        return Array.from(this.references)
            .filter(
                ([_, reference]) => !reference.resolved && !reference.requested,
            )
            .map(([alias, _]) => alias);
    }
}
...
export class Parser {
    ...
    seek(
        relOffset: ParserOptions["length"],
        options: ParserOptions = {},
    ): this {
        if (options.assert) {
    ...
    wrapped(varName: string | ParserOptions, options?: ParserOptions): this {
        ...
        if (!options || !options.wrapper || !options.type) {
            throw new Error(
                "Both wrapper and type must be defined for wrapped.",
            );
        }
    ...
    nest(varName: string | ParserOptions, options?: ParserOptions): this {
        ...
        if (
            !(options.type instanceof Parser) &&
            !aliasRegistry.has(options.type)
        ) {
            throw new Error(
                "type must be a known parser name or a Parser object.",
            );
        }
    ...
    sizeOf(): number {
        ...
        if (Object.keys(PRIMITIVE_SIZES).indexOf(this.type) >= 0) {
            ...
            if (typeof this.options.type === "string") {
                elementSize =
                    PRIMITIVE_SIZES[this.options.type as PrimitiveTypes];
    ...
    private encodeValue(
        ...
        if (this.type === "buffer") {
            const val = object[this.varName];
            const length = this.options.length as number;

            if (val.length !== length) {
                throw new Error(
                    `Field "${this.varName}" should be a buffer of length ${length}`,
                );
            }

            if (offset + length > dataView.byteLength) {
                throw new Error(
                    `Insufficient buffer size to encode field "${
                        this.varName
                    }". Expected at least ${offset + length} bytes, but got ${
                        dataView.byteLength
                    }.`,
                );
            }

            new Uint8Array(dataView.buffer, offset, length).set(val);

            if (this.next) {
                return this.next.encodeValue(dataView, object, offset + length);
            }
        ...
        if (this.type === "string") {
            ...
            for (const item of arr) {
                if (typeof this.options.type === "string") {
                    ...
                    newOffset = parser.encodeValue(
                        dataView,
                        { value: item },
                        newOffset,
                    );
                } else if (this.options.type instanceof Parser) {
                    newOffset = this.options.type.encodeValue(
                        dataView,
                        item,
                        newOffset,
                    );
    ...
    private nextNotBit() {
        ...
        if (this.next) {
            if (this.next.type === "nest") {
                ...
                if (
                    this.next.options &&
                    this.next.options.type instanceof Parser
                ) {
            ...
            ctx.bitFields.forEach((parser, i) => {
                ...
                if (length > rem) {
                    if (rem) {
                        const mask = -1 >>> (32 - rem);
                        ctx.pushCode(
                            `${parser.varName} = (${val} & 0x${mask.toString(
                                16,
                            )}) << ${length - rem};`,
                        );
                        length -= rem;
    ...
    private generateBuffer(ctx: Context) {
        ...
        if (typeof this.options.readUntil === "function") {
            ...
            ctx.pushCode(
                `${varName} = buffer.subarray(offset, offset + ${len});`,
            );
            ctx.pushCode(`offset += ${len};`);
        }
    ...
    private generateArray(ctx: Context) {
        ...
        if (typeof type === "string") {
            if (!aliasRegistry.get(type)) {
                ...
                const littleEndian =
                    PRIMITIVE_LITTLE_ENDIANS[type as PrimitiveTypes];
                ctx.pushCode(
                    `var ${item} = dataView.get${typeName}(offset, ${littleEndian});`,
                );
                ctx.pushCode(
                    `offset += ${PRIMITIVE_SIZES[type as PrimitiveTypes]};`,
                );
            } else {
                const tempVar = ctx.generateTmpVariable();
                ctx.pushCode(
                    `var ${tempVar} = ${FUNCTION_PREFIX + type}(offset, {`,
                );
    ...
    private generateChoice(ctx: Context) {
        ...
        if (this.options.defaultChoice) {
            ...
            ctx.generateError(
                `"Met undefined tag value " + ${tag} + " at choice"`,
            );
        }
        ctx.pushCode("}");
    ...
    private generateWrapper(ctx: Context) {
        ...
        if (typeof this.options.readUntil === "function") {
            ...
            const len = ctx.generateOption(this.options.length!);
            ctx.pushCode(
                `${wrappedBuf} = buffer.subarray(offset, offset + ${len});`,
            );
            ctx.pushCode(`offset += ${len};`);
        }
        ...
        if (this.options.clone) {
            ctx.pushCode(
                `${wrappedBuf} = buffer.constructor.from(${wrappedBuf});`,
            );
        }
    ...
    private generatePointer(ctx: Context) {
        ...
        if (this.options.type instanceof Parser) {
            ...
            Object.keys(PRIMITIVE_SIZES).indexOf(this.options.type!) >= 0
        ) {
            const typeName = PRIMITIVE_NAMES[type as PrimitiveTypes];
            const littleEndian =
                PRIMITIVE_LITTLE_ENDIANS[type as PrimitiveTypes];
