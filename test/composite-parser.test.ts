    ...
    describe(`Composite parser (${name})`, () => {
        ...
        describe("Array parser", () => {
            ...
            it("should parse associative arrays", () => {
                ...
                const buffer = factory([
                    0x02, 0xd2, 0x04, 0x00, 0x00, 0x2e, 0x16, 0x00, 0x00, 0x41,
                    0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x2e, 0x16, 0x00,
                    0x00, 0xd2, 0x04, 0x00, 0x00, 0x62, 0x62, 0x62, 0x62, 0x62,
                    0x62, 0x62, 0x62,
                ]);
                deepStrictEqual(parser.parse(buffer), {
                    numlumps: 2,
        ...
        describe("Choice parser", () => {
            ...
            it("should parse choices of user defined types", () => {
                ...
                const buffer1 = factory([
                    0x1, 0xc, 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x2c, 0x20, 0x77,
                    0x6f, 0x72, 0x6c, 0x64,
                ]);
          ...
          /* -> */ 0,
            ...
            it("should be able to use function as the choice selector", () => {
                ...
                const buffer1 = factory([
                    48, 48, 49, 48, 0xc, 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x2c,
                ...
                const buffer2 = factory([
                    48, 49, 49, 49, 0x4e, 0x61, 0xbc, 0x00,
                ]);
                deepStrictEqual(parser.parse(buffer2), {
            ...
            it("should be able to use parsing context", () => {
                ...
                const buffer1 = factory([
                    0x1, 0x2, 0xc, 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x2c, 0x20,
                    0x77, 0x6f, 0x72, 0x6c, 0x64, 0x01, 0x02, 0x02, 0x02,
                ]);
                deepStrictEqual(parser.parse(buffer1), {
                    tag: 1,
                    items: 2,
        ...
        describe("Nest parser", () => {
            it("should parse nested parsers", () => {
                ...
                const name = Array.from(
                    new TextEncoder().encode("John\0Doe\0"),
                );
                const age = [0x20];
            ...
            it("should be able to use parsing context", () => {
                ...
                const buffer = factory([
                    0x2, 0xc, 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x2c, 0x20, 0x77,
                    0x6f, 0x72, 0x6c, 0x64, 0x01, 0x02, 0x02, 0x02,
                ]);
                deepStrictEqual(parser.parse(buffer), {
                    items: 2,
                    data: {
        ...
        describe("Wrapper", () => {
            it("should parse deflated then inflated data", () => {
                ...
                const buffer = factory([
                    ...Array.from(
                        new Uint8Array(
                            new Uint32Array([compressedData.length]).buffer,
                        ),
                    ),
